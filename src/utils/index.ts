    ...
    if (get(propsConfig, k)) {
      if (!isObject(v)) {
        if (SPECIAL_STRING_CONSTANTS[v] !== undefined) {
          ...
          set(propsConfig,`${k}.placeholder`,v);
          const funcTemplate = get(propsConfig, `${k}.placeholder`);
          let splitArr = funcTemplate.split('.');
          splitArr.shift();//去掉this.
          console.log(splitArr);
          let join = splitArr.join('.');
          let funBodyArr = join.split('=');
          funBodyArr.shift();//去掉方法名
          const funBody = funBodyArr.join('=');

          set(propsConfig,`${k}.placeholder`,funBody);
          let argsString = funBody.toString().trim().split('=>')[0];
          let argsArr = argsString.split('(')[1].split(')')[0].split(',');
          let funBodyString = funBody.toString().trim().split('=>')[1];
          if (funBody) {
            // nextProps[k] = () => eval('(event)=>console.log("1111")');
            nextProps[k] = new Function(argsArr,funBodyString);
          } else {
            nextProps[k] = () => {
            };
          }
